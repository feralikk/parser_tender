...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            records = soup.find_all("div", "row", id=True)
            for record in records:
                title = record.find("div", "new_header_lwl2")
                link = "https://xn--22-9kcqjffxnf3b.xn--p1ai" + title.find('a').get('href')
                date_string = record.find("div", "new_data")
                date_diff = datetime.strptime(date_string.text, "%d.%m.%Y").date()
                records_list.append({'title': title.text.strip(),
                                    'link': link, 'date': date_string.text})
