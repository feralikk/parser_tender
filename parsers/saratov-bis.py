...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        year_links = soup.find_all("div", "documents_item")
        for year_link in year_links:
            date_string = year_link.text
            records_url = "https://saratov-bis.ru/" + year_link.find("a").get('href')
            response = session.get(records_url, timeout=5)
            if response.status_code == 200:
                soup = BeautifulSoup(response.text, 'lxml')
                records = soup.find_all("div", "documents_item")
                for record in records:
                    if not record.text.isspace():
                        title = record.text
                    else:
                        continue
                    try:
                        link = "https://saratov-bis.ru/" + record.find("a").get('href')
                    except AttributeError:
                        link = 'Ссылка отсутствует'
                    records_list.append({'title': title.strip(), 'link': link, 'date': date_string})
