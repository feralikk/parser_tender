...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("table", "table table-bordered table-striped")[1].find_all("tr")[1:]
        for record in records:
            title = record.find_all("td")[0].text.strip() + " " + record.find_all("td")[1].text.strip()
            link = "https://mbpenza.ru" + record.find_all("td")[1].find("a").get("href")
            date_string = record.find_all("td")[2].text
            records_list.append({'title': title, 'link': link, 'date': date_string.strip()})
