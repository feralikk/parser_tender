...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("div", "contractor__inner py-5 position-relative")
        for record in records:
            try:
                title = record.find("h3").text.strip()
            except AttributeError:
                title = record.find("div", "col-xl-8 col-md-7 mt-md-0 mt-2").text.strip()
            link = "Ссылка отсутствует"
            date_string = record.find("div", "col-xl-1 col-md-2").text
