...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find("table", "tender-table").find("tbody").find_all("tr")
        for record in records:
            title = record.find_all("td")[1].text.strip()
            link = "https://xn---24-9cdulgg0aog6b.xn--p1ai" + record.find_all("td")[3].find("a").get('href')
            date_string = record.find_all("td")[2].text.replace("\n\n\n", "\n")
