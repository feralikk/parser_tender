...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            records = soup.find_all("div", "elementor-post__card")
            for record in records:
                title = record.find("p")
                link = record.find('h3', 'elementor-post__title').find('a').get('href')
                date_string = record.find("span", "elementor-post-date")
                date_diff = datetime.strptime(date_string.text.strip(), "%d.%m.%Y").date()
                records_list.append({'title': title.text.strip(),
                                    'link': link, 'date': date_string.text.strip()})
