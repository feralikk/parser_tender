...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("div", "news__box")
        for record in records:
            title = record.find("a", "news__name")
            link = "https://mybusiness69.ru" + record.find("a", "news__name").get('href')
            records_list.append({'title': title.text.strip(), 'link': link, 'date': 'отсутсвует'})
