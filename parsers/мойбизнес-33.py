...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("a", "fade-in-animation item pb-3")
        for record in records:
            title = record.find("h3", "text-black1-color circle-font text-bold fs18")
            link = "https://xn---33-9cdulgg0aog6b.xn--p1ai" + record.get('href')
            date_string = record.find("span", "date text-white p-05 bg-begue").text.replace("\n", " ")
            ...
            date_diff = datetime.strptime(date_string.strip(), "%d %B %Y").date()
