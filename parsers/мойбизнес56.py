...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            records = soup.find_all("a", "unit-media zakup-contain-custom")
            for record in records:
                title = record.find("div", "unit-media-title")
                link = "https://xn--56-9kcqjffxnf3b.xn--p1ai" + record.get('href')
                date_string = record.find("div", "date-new").text
