...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            records = soup.find_all("div", "body-info")
            for record in records:
                title = record.find("div", "title")
                link = "https://export34.ru" + title.find('a', 'dark-color').get('href')
                date_string = record.find("div", "period").text
                ...
                try:
                    date_diff = datetime.strptime(date_string.rsplit(' – ')[1].strip(), "%d %B %Y %H:%M").date()
                except IndexError:
