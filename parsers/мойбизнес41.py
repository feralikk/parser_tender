...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            max_page = int(soup.find("ul", "panel_page alCenter").find_all("li")[-2].text)
            records = soup.find_all("div", "docs-detail__item")
            for record in records:
                title = record.find("div", "docs-detail__item-title")
                if (title.text.find("Приём заявок закрыт!") != -1) or title.text.find("Приём заявок\xa0закрыт!") != -1:
                    continue
                else:
                    records_list.append({'title': title.text.strip(), 'link': "", 'date': ""})
