...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            max_page = int(soup.find("div", "pagination__container").find_all("a")[-1].text)
            records = soup.find_all("div", "section-events__post event-post")
            for record in records:
                title = record.find("div", "event-post__title event-post__title_size-big")
                link = "https://kr-rk.ru/" + record.find("div", "event-post__head-text").find("a", "link-more").get('href')
                date_string = record.find("div", "event-post__subtitle-date").text
                date_string = re.sub(r'\s+', ' ', date_string)
                records_list.append({'title': title.text.strip(), 'link': link, 'date': date_string.strip()})
                date_diff = datetime.strptime(date_string[date_string.find('-')+1:].strip(), "%d.%m.%Y").date()
