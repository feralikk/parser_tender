...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("div", "concourses__item current-item")
        for record in records:
            title = record.find("div", "concourses__item-title")
            link = "https://xn--75-9kcqjffxnf3b.xn--p1ai" + record.find("a", "btn btn--more").get('href')
            date_string = record.find("div", "concourses__item-date").text.strip()
