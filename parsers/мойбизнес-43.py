...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find("div", "b-table-static").find("tbody").find_all("tr")
        for record in records:
            title = ''
            date_string = ''
            title_list = record.find_all("td")[0:2]
            for elem in title_list:
                title += elem.text + ' '
            link = "https://xn---43-9cdulgg0aog6b.xn--p1ai" + record.find("a").get('href')
            date_string += 'C ' + record.find_all("td")[2].text.replace('\n', ' ') + ' по ' + record.find_all("td")[2].text.replace('\n', ' ')
