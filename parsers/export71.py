...
def parse(url):
    ...
    while page != 0:
        ...
        if response.status_code == 200:
            ...
            records_list = []
            records = soup.find_all("div", "training-box tender-box")
            for record in records:
                title = record.find("span", "name")
                link = "https://export71.ru" + record.find('a').get('href')
                date_string = record.find("span", "t_date").text
