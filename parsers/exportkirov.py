...
def parse(url):
    ...
    if response.status_code == 200:
        ...
        records_list = []
        records = soup.find_all("div", "events-item column-3")
        for record in records:
            title = record.find("div", "events-item-title")
            link = "https://exportkirov.ru" + record.find('a', 'events-item-more').get('href')
            date_string = record.find("div", "events-item-date").text
